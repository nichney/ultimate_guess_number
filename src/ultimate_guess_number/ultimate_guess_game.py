...
class Guesser(cmd.Cmd):
    intro = (
        f"Welcome to the Ultimate Number Guessing Game!\nI'm thinking of a number between 1 and 100"
    ...
    def update_record(self, attempts, game_time):
        self.record[self.difficulty.name]["best_attempts"] = min(
            self.record[self.difficulty.name]["best_attempts"], attempts
        )
        self.record[self.difficulty.name]["worst_attempts"] = max(
            self.record[self.difficulty.name]["worst_attempts"], attempts
        )
        self.record[self.difficulty.name]["best_time"] = min(
            self.record[self.difficulty.name]["best_time"], game_time
        )
        self.record[self.difficulty.name]["worst_time"] = max(
            self.record[self.difficulty.name]["worst_time"], game_time
        )
    ...
    def default(self, guess):
        ...
        print(
            f"Incorrect! The number is {hint} than {guess}. {self.guesses_remain} guesses remaining."
        )
